...
public class CPHInline
{
    ...
    // Real chat messages are rarely over 200 chars. Spam scripts are 300-600+.
    private const int LENGTH_SUSPICIOUS  = 200;  // triggers as a multiplier with weak signals
    private const int LENGTH_WALL        = 350;  // long enough to be a script on its own
    private const int LENGTH_INSTANT_BAN = 500;  // no legitimate chat message is this long
    ...
    public bool Execute()
    {
        ...
        // ---- Platform inference (Kick only) ----
        if (IsBlank(platform))
        {
            if (!IsBlank(source) && source.IndexOf("kick", StringComparison.OrdinalIgnoreCase) >= 0)
                platform = "kick";
        }

        CPH.LogInfo("[SpamFilter-Kick] platform=" + platform
            + " | source=" + source
            + " | user=" + userName
            + " | msgId=" + msgId
        ...
        {
            CPH.LogInfo("[SpamFilter-Kick] Non-Kick platform detected. Ignoring.");
            return true;
        }
        ...
        {
            CPH.LogInfo("[SpamFilter-Kick] BYPASS allowlist: " + userName);
            return true;
        }
        ...
        {
            CPH.LogInfo("[SpamFilter-Kick] BAN botbanlist: " + userName);
            ExecuteKickBan(userName, "Known spam bot (auto)");
            return true;
        }

        if (IsTrustedRole(platform))
        {
            CPH.LogInfo("[SpamFilter-Kick] BYPASS trusted role: " + userName);
            return true;
        }
        ...
        CPH.LogInfo("[SpamFilter-Kick] norm=" + (norm.Length > 180 ? norm.Substring(0, 180) + "..." : norm));
        ...
        CPH.LogInfo("[SpamFilter-Kick] msgLen=" + msgLen
        ...
        bool streamerTargeting =
            HasAny(norm, "streamer", "streamers", "your stream", "your channel", "your kick");
        ...
        bool newStreamerExploit =
            HasAny(norm,
                "new in kick", "new on kick", "new streamer", "new channel",
        ...
        bool monetizationScam =
            ...
            (contactFunnel && HasAny(norm,
                "kick community", "share your stream", "share your channel", "gain 500", "active audience")) ||
        ...
        if (mentionsDiscord)
            CPH.LogInfo("[SpamFilter-Kick] DISCORD mention flagged for review | user=" + userName
                + " | msgLen=" + msgLen);
        ...
        // Wall-of-text + any Discord presence.
        ...
        bool bigAccountClaim =
            ...
            HasAny(norm,
                "top streamer", "top twitch streamer", "top kick streamer",
                "top twitch", "top kick",
        ...
        bool directGrowthReferralScam =
            HasAny(norm,
                "top twitch streamer", "top kick streamer",
        ...
        CPH.LogInfo("[SpamFilter-Kick] bigAccountClaim=" + bigAccountClaim
        ...
        // single strongest spam tell. The first-message flag is read from args (if the
        // Kick event provides it). Account age is read from args if a preceding "Get
        // User Info" sub-action set them; otherwise it is looked up via the API (best-
        // effort, reflection-based so it compiles on any Streamer.bot version and never
        // throws fatally). If none of this data is available the weighting stays off.
        ...
        CPH.LogInfo("[SpamFilter-Kick] firstMessage=" + isFirstMessage
        ...
        CPH.LogInfo("[SpamFilter-Kick] shouldBan=" + shouldBan
        ...
        if (IsBlank(userName))
        {
            CPH.LogInfo("[SpamFilter-Kick] Spam detected but username was blank. Cannot ban.");
            return true;
        }

        CPH.LogInfo("[SpamFilter-Kick] BANNING " + userName);
        ExecuteKickBan(userName, "Spam detected (auto)");
    ...
    private void ExecuteKickBan(string userName, string reason)
    {
        bool banned = false;

        try
        {
            banned = CPH.KickBanUser(userName, reason, true);
        }
        catch (Exception ex)
        {
            CPH.LogInfo("[SpamFilter-Kick] Kick ban error: " + ex.Message);
        }

        CPH.LogInfo("[SpamFilter-Kick] ban=" + banned
            + " | user=" + userName
            + " | reason=" + reason);
    }
    ...
    private bool IsTrustedRole(string platform)
    {
        if (platform.Equals("kick", StringComparison.OrdinalIgnoreCase))
        {
            ...
            string role = GetArg("role", GetArg("userRole", ""));
            if (!IsBlank(role))
            {
                string r = role.ToLowerInvariant();

                if (Regex.IsMatch(r, @"(^|[,\s;|])mod(erator)?([,\s;|]|$)"))
                    isMod = true;

                if (Regex.IsMatch(r, @"(^|[,\s;|])vip([,\s;|]|$)"))
                    isVip = true;
            }
            ...
            if (!IsBlank(badges))
            {
                ...
                if (Regex.IsMatch(b, @"(^|[,\s;|])mod(erator)?([,\s;|]|$)"))
                    isMod = true;
            ...
            CPH.LogInfo("[SpamFilter-Kick] trustedCheck kick"
                + " | mod=" + isMod
                + " | vip=" + isVip
                + " | role=" + role
                + " | badges=" + badges);
    ...
    private double GetAccountAgeDays(string userId, string userLogin)
    {
        ...
        DateTime created;
        if (TryParseDate(GetArg("accountCreated", GetArg("createdAt", GetArg("channelCreatedAt", ""))), out created))
            return (DateTime.UtcNow - created.ToUniversalTime()).TotalDays;

        // 2) Best-effort API lookup via reflection. Tries several possible Streamer.bot
        //    Kick user-info method names; if none exist in this version it simply
        //    returns -1, so the script always compiles and the weighting stays inactive.
        try
        {
            object info =
                InvokeCph("KickGetUserInfo", userLogin) ??
                InvokeCph("KickGetUser", userLogin) ??
                InvokeCph("KickGetUserByLogin", userLogin) ??
                InvokeCph("KickGetUserById", userId);

            if (info != null)
            {
                DateTime? c = ReadDateProperty(info,
                    "Created", "AccountCreated", "CreatedAt", "ChannelCreatedAt",
                    "CreatedAtRfc3339", "UserCreated");
                if (c.HasValue)
        ...
        {
            CPH.LogInfo("[SpamFilter-Kick] account-age lookup skipped: " + ex.Message);
        }
